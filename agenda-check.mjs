  ...
  function openFloatingMenu(button) {
    ...
    const menuWidth = Math.max(menuRect.width || 210, 210);
    const menuHeight = Math.max(menuRect.height || 220, 180);
    const gutter = 12;
    const left = Math.min(window.innerWidth - menuWidth - gutter, Math.max(gutter, rect.right - menuWidth));
    let top = rect.bottom + 8;
    if (top + menuHeight > window.innerHeight - gutter) top = Math.max(gutter, rect.top - menuHeight - 8);
