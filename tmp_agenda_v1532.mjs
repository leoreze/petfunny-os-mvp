  ...
  function closeFloatingMenus(except = null) {
    document.querySelectorAll('.card-menu.is-open-floating').forEach(menu => {
      if (menu === except) return;
      menu.classList.remove('is-open-floating');
  ...
  function openFloatingMenu(button) {
    const wrap = button.closest('.card-kebab-wrap');
    const menu = wrap?.querySelector('.card-menu');
    if (!menu) return;
    const alreadyOpen = menu.classList.contains('is-open-floating');
    ...
    const top = Math.min(window.innerHeight - 250, rect.bottom + 8);
    menu.style.position = 'fixed';
    menu.style.left = `${left}px`;
    menu.style.top = `${Math.max(12, top)}px`;
    menu.style.right = 'auto';
    menu.style.zIndex = '2147483000';
    menu.classList.add('is-open-floating');
  }

  function appointmentActionsMenu(a) {
    const receiptButton = isPaymentPaid(a) ? `<button class="view-receipt-modal" data-id="${esc(a.id)}" type="button">Recibo</button>` : '';
    return `<div class="card-kebab-wrap">
      <button class="kebab-btn kebab" type="button" aria-label="Abrir ações do agendamento">⋯</button>
      <div class="card-menu">
        <button class="edit-appointment" data-id="${esc(a.id)}" type="button">Editar</button>
        <button class="status-action" data-id="${esc(a.id)}" data-status="cancelado" type="button">Cancelar</button>
        <button class="view-command-modal" data-id="${esc(a.id)}" type="button">Comanda</button>
        ${receiptButton}
        <button data-whatsapp-action="confirmacao_agendamento" data-appointment-id="${esc(a.id)}" type="button">WhatsApp confirmação</button>
      </div>
    </div>`;
  }
  ...
  function renderWeek() {
    ...
    nodes.week.innerHTML = '<div class="time-cell">Hora</div>' + days.map(d => `<div class="time-cell"><strong>${new Date(`${d}T12:00:00`).toLocaleDateString('pt-BR',{weekday:'short',day:'2-digit'})}</strong>${slotTag(d)}</div>`).join('') + times.map(t => `<div class="time-cell"><strong>${esc(t)}</strong></div>${days.map(d => {
  ...
  document.addEventListener('click', async e => {
    ...
    if (!e.target.closest('.card-menu')) closeFloatingMenus();
