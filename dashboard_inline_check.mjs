    ...
    const DASHBOARD_LOADING_TIMEOUT_MS = 45000;
    const DASHBOARD_OPTIONAL_TIMEOUT_MS = 18000;

    function withClientTimeout(promise, timeoutMs, label = 'requisição') {
      let timer;
      const timeout = new Promise((_, reject) => {
        timer = window.setTimeout(() => reject(new Error(`${label} demorou demais para responder.`)), timeoutMs);
      });
      return Promise.race([promise, timeout]).finally(() => window.clearTimeout(timer));
    }

    async function waitForDashboardVisualReady() {
      await waitForPagePaintAndImages();
      await new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve)));
      await new Promise((resolve) => setTimeout(resolve, 80));
    }

    function fallbackAiGrowthPlan(reason = 'O plano de crescimento será atualizado quando a API responder.') {
      return {
        ...
        plan: {
          ...
          dateLabel: 'Hoje',
          score: 52,
          mood: 'fallback seguro',
          mainGoal: 'Manter operação, agenda e financeiro sob controle enquanto a IA conclui a análise.',
          diagnosis: reason,
          tasks: [
            { title: 'Revisar agenda do dia', description: 'Confira horários, status, atrasos e pagamentos antes de iniciar os atendimentos.', priority: 'alta', module: 'Agenda', due: 'Agora', effort: '10 min', kpi: 'agenda conferida', route: '/admin/agenda' },
            { title: 'Baixar recebimentos pendentes', description: 'Abra o financeiro e confira pagamentos em aberto para não fechar o dia com caixa distorcido.', priority: 'alta', module: 'Financeiro', due: 'Hoje', effort: '15 min', kpi: 'pendências reduzidas', route: '/admin/financeiro' }
          ],
          campaigns: [],
          risks: ['A IA demorou para responder e o dashboard usou o plano seguro local.'],
          opportunities: ['Atualize somente a IA pelo botão do card depois que a página estiver aberta.'],
          routine: { morning: 'Conferir agenda e confirmações.', afternoon: 'Registrar atendimentos e vender próximo banho.', closing: 'Dar baixa financeira e preparar reativações.' }
        }
      };
    }

    function fallbackEngagementDashboard(reason = 'Engajamento 360 será exibido quando a rota estiver disponível.') {
      return { metrics: {}, retention: [], rewards: [], topTutors: [], insights: [reason] };
    }


    const ADMIN_MODULE_ROUTES = [
      { route: '/admin/agenda', terms: ['agenda', 'agendamento', 'horário', 'horario', 'encaixe', 'confirmar'] },
      { route: '/admin/tutores', terms: ['tutor', 'tutores', 'cliente', 'clientes', 'responsável', 'responsavel'] },
      { route: '/admin/pets', terms: ['pet', 'pets', 'animal'] },
      { route: '/admin/servicos', terms: ['serviço', 'servico', 'serviços', 'servicos', 'banho', 'tosa', 'hidratação', 'hidratacao'] },
      { route: '/admin/pacotes', terms: ['pacote', 'pacotes', 'recorrência', 'recorrencia', 'renovação', 'renovacao', 'sessão', 'sessao'] },
      { route: '/admin/assinaturas', terms: ['assinatura', 'assinaturas'] },
      { route: '/admin/financeiro', terms: ['financeiro', 'pagamento', 'recebimento', 'pendência', 'pendencia', 'caixa', 'faturamento', 'cobrança', 'cobranca'] },
      { route: '/admin/comandas-recibos', terms: ['comanda', 'recibo', 'documento'] },
      { route: '/admin/crm', terms: ['crm', 'marketing', 'lead', 'reativação', 'reativacao', 'campanha', 'whatsapp comercial'] },
      { route: '/admin/promocoes', terms: ['promoção', 'promocao', 'promoções', 'promocoes', 'oferta', 'desconto'] },
      { route: '/admin/bem-estar', terms: ['petfunny 360', 'diagnóstico', 'diagnostico', 'avaliação', 'avaliacao', 'bem-estar', 'bem estar'] },
      { route: '/admin/saude-360', terms: ['saúde', 'saude', 'teleconsulta', 'veterinário', 'veterinario'] },
      { route: '/admin/roleta-de-mimos', terms: ['roleta', 'mimo', 'mimos', 'sorteio', 'brinde'] },
      { route: '/admin/relatorios', terms: ['relatório', 'relatorio', 'relatórios', 'relatorios', 'indicador', 'kpi'] },
      { route: '/admin/notificacoes', terms: ['notificação', 'notificacao', 'notificações', 'notificacoes', 'alerta', 'push'] },
      { route: '/admin/app-acessos', terms: ['app do tutor', 'acessos do app', 'app', 'engajamento', 'momentos', 'foto', 'fotos', 'vídeo', 'video', 'timeline'] },
      { route: '/admin/whatsapp', terms: ['whatsapp', 'mensagem', 'mensagens'] },
      { route: '/admin/assistente-ia', terms: ['ia', 'assistente', 'gerente ia'] },
      { route: '/admin/configuracoes', terms: ['configuração', 'configuracao', 'configurações', 'configuracoes', 'capacidade'] }
    ];
    const ADMIN_ALLOWED_ROUTES = new Set(['/admin/dashboard', ...ADMIN_MODULE_ROUTES.map(item => item.route)]);

    function normalizeAdminRoute(route = '') {
      const value = String(route || '').trim();
      if (!value) return '';
      const pathWithQuery = value.replace(/^https?:\/\/[^/]+/i, '').startsWith('/') ? value.replace(/^https?:\/\/[^/]+/i, '') : `/${value.replace(/^https?:\/\/[^/]+/i, '')}`;
      const [pathname, query = ''] = pathWithQuery.split('?');
      const normalizedPath = pathname.replace(/\/+/g, '/').replace(/\/$/, '') || '/admin/dashboard';
      if (!ADMIN_ALLOWED_ROUTES.has(normalizedPath)) return '';
      return query ? `${normalizedPath}?${query.slice(0, 80)}` : normalizedPath;
    }

    function getAiTaskRoute(task = {}) {
      const direct = normalizeAdminRoute(task.route || task.rota);
      ...
      const found = ADMIN_MODULE_ROUTES.find(item => item.terms.some(term => text.includes(term)));
      return found?.route || '/admin/dashboard';
    }
    ...
    function openDashboardActionMenu(button) {
      ...
      menu.innerHTML = `
        ...
        ${receiptButton}
        <button data-dashboard-menu-action="message" data-id="${esc(id)}" type="button">✨ Enviar mensagem</button>
      `;
    ...
    async function openDashboardCommand(id) {
      const item = itemById(id) || {};
      const url = item.customerPackageId ? `/documentos/pacote-comanda/${encodeURIComponent(item.customerPackageId)}` : `/documentos/comanda/${encodeURIComponent(id)}`;
    ...
    async function openDashboardReceipt(id) {
      try {
        ...
        if (item.customerPackageId) {
          window.open(`/documentos/pacote-recibo/${encodeURIComponent(item.customerPackageId)}`, '_blank', 'noopener');
          return;
        }
    ...
    function dashboardMessageModalRefs() {
      return {
        modal: document.getElementById('dashboard-status-message-modal'),
        eyebrow: document.getElementById('dashboard-status-message-eyebrow'),
        title: document.getElementById('dashboard-status-message-title'),
        subtitle: document.getElementById('dashboard-status-message-subtitle'),
        context: document.getElementById('dashboard-status-message-context'),
        text: document.getElementById('dashboard-status-message-text'),
        close: document.getElementById('close-dashboard-status-message-modal'),
        cancel: document.getElementById('cancel-dashboard-status-message'),
        send: document.getElementById('send-dashboard-status-message-whatsapp')
      };
    }

    function dashboardStatusMessageContextHtml(payload = {}) {
      const a = payload.appointment || {};
      const status = payload.status || {};
      const payment = [a.paymentStatusName || paymentText(a), a.paymentMethodName].filter(Boolean).join(' · ');
      return `<div class="ai-message-context-grid">
        ...
        <div><span>Horário</span><strong>${esc(a.startsAt ? new Date(a.startsAt).toLocaleString('pt-BR', { dateStyle:'short', timeStyle:'short' }) : '—')}</strong></div>
        <div><span>Serviço</span><strong>${esc(a.services || '—')}</strong></div>
        <div><span>Pagamento</span><strong>${esc(payment || 'A definir')}</strong></div>
      </div>`;
    }

    async function openDashboardStatusMessage(id, status = '') {
      if (!id) return;
      const refs = dashboardMessageModalRefs();
      if (!refs.modal) return;
      try {
        refs.eyebrow.textContent = 'Assistente Inteligente PetFunny';
        refs.title.textContent = 'Preparando mensagem';
        refs.subtitle.textContent = 'A mensagem será criada com os dados do atendimento e o contexto do status.';
        refs.context.innerHTML = '<div class="document-loading-state compact"><img src="/assets/img/loading-dog.gif" alt="Carregando"><strong>Gerando sugestão...</strong><small>Organizando tutor, pet, horário e atendimento.</small></div>';
        refs.text.value = '';
        refs.send.href = '#';
        refs.send.classList.add('disabled');
        refs.modal.classList.add('is-loading');
        ...
        refs.title.textContent = `Mensagem sobre ${statusNameLabel}`;
        refs.subtitle.textContent = 'Revise a sugestão. Ao clicar em enviar, o WhatsApp abre com o texto pronto para a equipe confirmar o envio.';
        refs.context.innerHTML = dashboardStatusMessageContextHtml(payload);
        refs.text.value = payload.message || '';
        refs.modal.classList.remove('is-loading');
        const updateLink = () => {
          ...
          const href = normalized ? `https://wa.me/${normalized}?text=${encodeURIComponent(refs.text.value || '')}` : `https://wa.me/?text=${encodeURIComponent(refs.text.value || '')}`;
          refs.send.href = href;
          refs.send.classList.remove('disabled');
        };
        refs.text.oninput = updateLink;
        updateLink();
      } catch (error) {
        refs.modal.classList.remove('is-loading');
        if (refs.modal.open) refs.modal.close();
        await showResultModal({ type: 'error', title: 'Mensagem indisponível', message: error.message || 'Não foi possível gerar a mensagem agora.', okText: 'OK' });
      }
    }

    function renderAgendaTable() {
      const tbody = document.querySelector('#agenda-table tbody');
      const cards = document.getElementById('agenda-dashboard-cards');
      ...
      if (cards) {
        cards.innerHTML = rows.length ? rows.map(agendaCard).join('') : emptyState('Nenhum atendimento encontrado', 'Ajuste a busca rápida ou confira a agenda do dia.');
      }
    ...
    function setAgendaDashboardView(view = 'list') {
      agendaDashboardView = view === 'cards' ? 'cards' : 'list';
      document.querySelectorAll('[data-agenda-dashboard-view]').forEach(btn => btn.classList.toggle('active', btn.dataset.agendaDashboardView === agendaDashboardView));
      const list = document.getElementById('agenda-dashboard-list-view');
      const cards = document.getElementById('agenda-dashboard-card-view');
      if (list) list.hidden = agendaDashboardView !== 'list';
      if (cards) cards.hidden = agendaDashboardView !== 'cards';
      renderAgendaTable();
    }
    ...
    function dashboardCalendarEvent(item, compact = false) {
      const color = appointmentStatusColor(item);
      const statusName = statusLabels[item.status] || item.status || 'Status';
      const packageBadge = item.packageSessionLabel ? `<span class="package-calendar-badge">📦 ${esc(item.packageSessionLabel)}</span>` : '';
      return `<article draggable="true" class="cal-event calendar-appointment-card" style="--status-color:${esc(color)}" data-id="${esc(item.id)}" data-start="${esc(item.startsAt)}">
        <div class="cal-event-main edit-appointment" data-id="${esc(item.id)}">
          ...
          <div class="cal-event-text"><strong>${compact ? `${esc(time(item.startsAt))} ${esc(item.petName || 'Pet')}` : `${esc(time(item.startsAt))} · ${esc(item.petName || 'Pet')}`} ${packageBadge}</strong><small>${esc(item.tutorName || 'Tutor')}${compact ? '' : ` · ${brl(item.totalCents)}`}<span class="payment-info-line">${esc(paymentText(item))}</span><span class="mini-status-name">${esc(statusName)}</span></small></div>
        </div>
    ...
    async function updateAppointmentStatus(id, status) {
      ...
      try {
        ...
        await loadDashboard();
        await openDashboardStatusMessage(id, status);
      } catch (error) {
    ...
    function engagementAdminCard(label, value, hint, icon, tone = 'info') {
      return `
        <article class="engagement-admin-kpi engagement-${tone}">
          <span class="engagement-admin-icon">${icon}</span>
          <small>${label}</small>
          <strong>${value}</strong>
          <p>${hint}</p>
        </article>`;
    ...
    function aiWhatsappHref(task = {}) {
      const phone = String(task.whatsappPhone || task.phone || task.whatsapp || '').replace(/\D/g, '');
      const normalized = phone ? (phone.startsWith('55') ? phone : `55${phone}`) : '';
      const message = String(task.whatsappMessage || '').trim();
    ...
    function renderAiTaskAction(task = {}) {
      const href = aiWhatsappHref(task);
      const label = task.actionLabel || 'Enviar WhatsApp';
      const client = [task.clientName, task.petName].filter(Boolean).join(' · ');
      if (!href) return '';
      return `<div class="ai-task-action">${client ? `<small>${esc(client)}</small>` : ''}<a class="ai-task-whatsapp" href="${esc(href)}" target="_blank" rel="noopener">💬 ${esc(label)}</a></div>`;
    }

    function renderAiGrowthPlan(summary) {
      const payload = summary.aiGrowthPlan || {};
      ...
      const score = Math.max(0, Math.min(100, Number(plan.score || 0)));
      const aiMode = payload.openaiUsed ? 'IA real conectada' : (payload.openaiConfigured ? 'IA local + fallback seguro' : 'Plano local seguro');
      const generatedAt = payload.generatedAt ? dateTime(payload.generatedAt) : 'agora';
      return `
        <section class="dashboard-full-row ai-growth-section">
          <article class="surface-panel ai-growth-panel">
            <div class="panel-heading ai-growth-heading">
              <div>
                <p class="eyebrow">Gerente IA de Crescimento</p>
                <h2>${esc(plan.title || 'Plano diário para crescer o PetFunny')}</h2>
                <p>${esc(plan.diagnosis || 'A IA analisa agenda, financeiro, pacotes, clientes e oportunidades para transformar dados em tarefas diárias.')}</p>
              </div>
              <div class="ai-growth-score" style="--score:${score}">
                <strong>${score}</strong>
                <small>${esc(plan.mood || 'em análise')}</small>
              </div>
            </div>
            <div class="ai-growth-meta">
              <span>📅 ${esc(plan.dateLabel || 'Hoje')}</span>
              <span>⚡ ${esc(aiMode)}</span>
              <span>🕒 Atualizado ${esc(generatedAt)}</span>
            </div>
            <div class="ai-growth-goal">
              <span>🎯 Objetivo do dia</span>
              <strong>${esc(plan.mainGoal || 'Aumentar agenda, recorrência e recebimentos hoje.')}</strong>
            </div>
            <div class="ai-growth-grid">
              <div class="ai-growth-main">
                <div class="section-toolbar compact"><div><h3>Ações e tarefas de hoje</h3><p>Checklist prático para a equipe executar e acompanhar.</p></div><button class="btn btn-secondary" id="copy-ai-growth-plan" type="button">Copiar plano</button></div>
                <div class="ai-task-list">
                  ${tasks.length ? tasks.map((task, index) => {
                    const taskRoute = getAiTaskRoute(task);
                    return `
                    <article class="ai-task-card priority-${esc(String(task.priority || 'media').toLowerCase().replace('é','e'))}">
                      <div class="ai-task-index">${String(index + 1).padStart(2, '0')}</div>
                      <div class="ai-task-content">
                        <div class="ai-task-title-row"><strong>${esc(task.title)}</strong><span>${priorityLabel(task.priority)}</span></div>
                        <p>${esc(task.description)}</p>
                        <div class="ai-task-tags"><small>📍 ${esc(task.module || 'Dashboard')}</small><small>⏱ ${esc(task.due || 'Hoje')}</small><small>💪 ${esc(task.effort || '15 min')}</small><small>📊 ${esc(task.kpi || 'crescimento')}</small></div>
                        ${renderAiTaskAction(task)}
                      </div>
                      <a class="ai-task-link" href="${esc(taskRoute)}" data-ai-task-route="${esc(taskRoute)}">Abrir módulo</a>
                    </article>`;
                  }).join('') : emptyState('Plano ainda não carregado', 'Atualize o painel para gerar as tarefas do dia.')}
                </div>
              </div>
              <aside class="ai-growth-side">
                <div class="ai-side-card">
                  <h3>Rotina sugerida</h3>
                  <p><b>Manhã:</b> ${esc(routine.morning || 'Confirmar agenda e olhar pendências.')}</p>
                  <p><b>Tarde:</b> ${esc(routine.afternoon || 'Acompanhar atendimentos e vender próximos banhos.')}</p>
                  <p><b>Fechamento:</b> ${esc(routine.closing || 'Baixar financeiro e preparar amanhã.')}</p>
                </div>
                <div class="ai-side-card">
                  <h3>Riscos</h3>
                  <div class="ai-mini-list">${risks.length ? risks.map(item => `<span>⚠️ ${esc(item)}</span>`).join('') : '<span>Nenhum risco crítico identificado.</span>'}</div>
                </div>
                <div class="ai-side-card">
                  <h3>Oportunidades</h3>
                  <div class="ai-mini-list">${opportunities.length ? opportunities.map(item => `<span>💡 ${esc(item)}</span>`).join('') : '<span>Sem oportunidades novas no momento.</span>'}</div>
                </div>
              </aside>
            </div>
            <div class="ai-campaign-block">
              <div class="section-toolbar compact"><div><h3>Campanhas prontas para WhatsApp</h3><p>Textos gerados para reativar, vender pacote e preencher agenda.</p></div><button class="btn" id="refresh-ai-growth-plan" type="button">Atualizar IA</button></div>
              <div class="ai-campaign-grid">
                ${campaigns.length ? campaigns.map((campaign) => `
                  <article class="ai-campaign-card">
                    <span>${esc(campaign.channel || 'WhatsApp')}</span>
                    <strong>${esc(campaign.title)}</strong>
                    <small>${esc(campaign.target || 'tutores')}</small>
                    <p>${esc(campaign.message || '')}</p>
                  </article>`).join('') : emptyState('Sem campanhas geradas', 'Configure dados de CRM e agenda para a IA montar campanhas mais específicas.')}
              </div>
            </div>
          </article>
        </section>`;
    ...
    function getAiGrowthPlanText() {
      const payload = currentSummary?.aiGrowthPlan || {};
      const plan = payload.plan || payload || {};
      const lines = [
        `${plan.title || 'Gerente IA de Crescimento PetFunny'} — ${plan.dateLabel || 'Hoje'}`,
        `Score: ${plan.score || '--'} · ${plan.mood || ''}`,
      ...
      (plan.tasks || []).forEach((task, index) => {
        lines.push(`${index + 1}. ${task.title} [${priorityLabel(task.priority)}]`);
        lines.push(`   ${task.description}`);
        lines.push(`   Prazo: ${task.due || 'Hoje'} · KPI: ${task.kpi || 'crescimento'}`);
      });
      if ((plan.campaigns || []).length) {
        lines.push('', 'Campanhas WhatsApp:');
        (plan.campaigns || []).forEach((campaign, index) => {
          lines.push(`${index + 1}. ${campaign.title} — ${campaign.message}`);
        });
      }
      return lines.join('\n').trim();
    }

    async function copyAiGrowthPlan() {
      const text = getAiGrowthPlanText();
      try {
        await navigator.clipboard.writeText(text);
        await showSuccessModal('Plano copiado', 'O plano diário da IA foi copiado para você compartilhar com a equipe.');
      } catch (error) {
        await showResultModal({ type: 'info', title: 'Plano diário da IA', message: text.slice(0, 1200), okText: 'OK' });
      }
    }

    async function refreshAiGrowthPlanOnly() {
      const btn = document.getElementById('refresh-ai-growth-plan');
      if (btn) btn.disabled = true;
      try {
        const payload = await api.get('/dashboard/ai-growth-plan');
        currentSummary.aiGrowthPlan = payload;
        const section = document.querySelector('.ai-growth-section');
        if (section) {
          section.outerHTML = renderAiGrowthPlan(currentSummary);
          bindDashboardInteractions();
        }
      } catch (error) {
        await showResultModal({ type: 'error', title: 'IA indisponível', message: error.message || 'Não foi possível atualizar o plano agora.', okText: 'OK' });
      } finally {
        const nextBtn = document.getElementById('refresh-ai-growth-plan');
        if (nextBtn) nextBtn.disabled = false;
      }
    }

    function renderEngagementDashboard(summary) {
      const dash = summary.engagementDashboard || {};
      const metrics = dash.metrics || {};
      const retention = dash.retention || [];
      const rewards = dash.rewards || [];
      const topTutors = dash.topTutors || [];
      const insights = dash.insights || [];
      const score = Number(metrics.appEngagementScore || 0);
      return `
        <section class="dashboard-full-row engagement-admin-section">
          <article class="surface-panel engagement-admin-panel">
            <div class="panel-heading">
              <div>
                <p class="eyebrow">Engajamento 360°</p>
                <h2>Retenção, recompensas e app do tutor</h2>
                <p>Resumo consolidado para acompanhar recorrência, ossinhos, indicações, clientes em risco e oportunidades de recompra.</p>
              </div>
              <span class="badge">Score ${score}/100</span>
            </div>
            <div class="engagement-admin-score">
              <div class="engagement-score-ring" style="--score:${Math.max(0, Math.min(100, score))}"><strong>${score}</strong><small>score app</small></div>
              <div>
                <h3>Home do Tutor orientada para recorrência</h3>
                <p>🐶 Pet ativo · 📅 próximo banho · 🩺 Health Score · 🦴 ossinhos · 🎁 mimos · 📸 momentos · IA de cuidados.</p>
                <div class="engagement-home-preview">
                  ${(dash.homePreview?.cards || ['Próximo banho','Health Score','Ossinhos','Mimos','Dica IA']).map(card => `<span>${esc(card)}</span>`).join('')}
                </div>
              </div>
            </div>
            <div class="engagement-admin-grid">
              ${engagementAdminCard('Tutores ativos', num(metrics.activeTutors), `${num(metrics.recurringTutors)} recorrente(s)`, '👤', 'info')}
              ${engagementAdminCard('Em risco', num((metrics.atRiskTutors || 0) + (metrics.lostTutors || 0)), 'sem agendar há muito tempo', '⚠️', 'warning')}
              ${engagementAdminCard('Ossinhos gerados', num(metrics.rewardPoints), `${num(metrics.rewardsEvents)} evento(s)`, '🦴', 'success')}
              ${engagementAdminCard('Indicações', num(metrics.referralsCreated), `${num(metrics.referralsConverted)} convertida(s)`, '🤝', 'info')}
              ${engagementAdminCard('Pacotes ativos', num(metrics.activePackages), 'base recorrente atual', '📦', 'success')}
              ${engagementAdminCard('Momentos', num(metrics.mediaItems), 'fotos/vídeos no app', '📸', 'info')}
            </div>
            <div class="engagement-admin-columns">
              <div>
                <h3>Retenção por última visita</h3>
                <div class="engagement-list">
                  ${retention.length ? retention.map(item => `<article><span>${esc(item.label)}</span><strong>${num(item.total)}</strong></article>`).join('') : emptyState('Sem dados de retenção', 'Os dados aparecem quando houver tutores e agendamentos.')}
                </div>
              </div>
              <div>
                <h3>Recompensas</h3>
                <div class="engagement-list">
                  ${rewards.length ? rewards.map(item => `<article><span>${esc(item.type)}</span><strong>${num(item.points)} pts</strong><small>${num(item.events)} evento(s)</small></article>`).join('') : emptyState('Sem ossinhos ainda', 'Quando o tutor agendar, indicar ou compartilhar, os eventos aparecem aqui.')}
                </div>
              </div>
              <div>
                <h3>Top tutores</h3>
                <div class="engagement-list">
                  ${topTutors.length ? topTutors.map(item => `<article><span>${esc(item.name)}</span><strong>${num(item.points)} 🦴</strong><small>${num(item.appointments)} atendimento(s)</small></article>`).join('') : emptyState('Sem ranking ainda', 'Os tutores com maior vínculo aparecem aqui.')}
                </div>
              </div>
            </div>
            <div class="engagement-insights">
              ${insights.map(item => `<article><span>✨</span><p>${esc(item)}</p></article>`).join('')}
            </div>
          </article>
        </section>`;
    }
    ...
    function renderDashboard(summary) {
      ...
      buildShell({
        ...
        content: `
          ...
          <section class="quick-actions-panel" aria-label="Atalhos rápidos da operação">
            <a class="quick-action-card" href="/admin/agenda?new=1">
              <span class="quick-action-icon">📅</span>
              <strong>Novo agendamento</strong>
              <small>Abra a agenda já preparado para cadastrar um horário.</small>
            </a>
            <a class="quick-action-card" href="/admin/tutores?new=1">
              <span class="quick-action-icon">👤</span>
              <strong>Novo tutor</strong>
              <small>Cadastre rapidamente um cliente e mantenha os contatos organizados.</small>
            </a>
            <a class="quick-action-card" href="/admin/pets?new=1">
              <span class="quick-action-icon">🐾</span>
              <strong>Novo pet</strong>
              <small>Inclua um pet com porte, raça, cuidados e vínculo com o tutor.</small>
            </a>
            <a class="quick-action-card" href="/admin/pacotes?sell=1">
              <span class="quick-action-icon">🎁</span>
              <strong>Vender pacote</strong>
              <small>Registre uma recorrência e gere as sessões futuras do atendimento.</small>
            </a>
          </section>
            ...
            <article class="surface-panel dashboard-main-panel">
              <div class="panel-heading agenda-day-heading">
                ...
                <div class="agenda-day-actions">
                  ...
                  <div class="segmented-control compact" role="group" aria-label="Visualização da agenda">
                    <button type="button" class="active" data-agenda-dashboard-view="list">Lista</button>
                    <button type="button" data-agenda-dashboard-view="cards">Cards</button>
                  </div>
                </div>
              </div>
              ...
              <div id="agenda-dashboard-list-view" class="table-wrap dashboard-table-wrap">
              ...
              <div id="agenda-dashboard-card-view" class="dashboard-agenda-cards" hidden><div id="agenda-dashboard-cards" class="dashboard-agenda-card-grid"></div></div>
            </article>
          </section>
          ...
          <section class="dashboard-full-row">
            <article class="surface-panel dashboard-upcoming-panel">
              <div class="panel-heading compact"><div><p class="eyebrow">Próximos horários</p><h2>Agenda dos próximos dias</h2></div><a class="btn btn-secondary" href="/admin/agenda">Abrir agenda</a></div>
          ...
          ${slotModalHtml()}
          ${dashboardStatusMessageModalHtml()}`
      });
    ...
    function dashboardStatusMessageModalHtml() {
      return `
        <dialog class="modal-shell agenda-message-modal" id="dashboard-status-message-modal">
          <form method="dialog" class="modal-card agenda-appointment-modal-card agenda-message-modal-card">
            <header class="modal-header agenda-modal-header">
              <div class="modal-title-block">
                <p class="eyebrow" id="dashboard-status-message-eyebrow">Mensagem inteligente</p>
                <h2 id="dashboard-status-message-title">Mensagem para o tutor</h2>
                <p class="modal-subtitle" id="dashboard-status-message-subtitle">Aguarde a sugestão e revise o texto antes de abrir o WhatsApp.</p>
              </div>
              <button class="icon-btn modal-close-btn" type="button" id="close-dashboard-status-message-modal" aria-label="Fechar mensagem">✕</button>
            </header>
            <div class="modal-body custom-scroll">
              <div class="ai-message-context" id="dashboard-status-message-context"></div>
              <label class="form-field">
                <span>Mensagem sugerida</span>
                <textarea class="textarea ai-message-textarea" id="dashboard-status-message-text" rows="8"></textarea>
              </label>
            </div>
            <footer class="modal-footer">
              <button class="btn btn-secondary" id="cancel-dashboard-status-message" type="button">Cancelar</button>
              <a class="btn" id="send-dashboard-status-message-whatsapp" target="_blank" rel="noopener">Enviar WhatsApp</a>
            </footer>
          </form>
        </dialog>`;
    }
        ...
        if (action) {
          ...
          if (type === 'cancel') {
            try {
              ...
              await loadDashboard();
              await openDashboardStatusMessage(id, 'cancelado');
            } catch (error) {
          ...
          if (type === 'message') return openDashboardStatusMessage(id);
          return;
        }
    ...
    function bindDashboardInteractions() {
      ...
      const msgRefs = dashboardMessageModalRefs();
      msgRefs.close?.addEventListener('click', () => msgRefs.modal?.close());
      msgRefs.cancel?.addEventListener('click', () => msgRefs.modal?.close());
      msgRefs.send?.addEventListener('click', () => setTimeout(() => msgRefs.modal?.close(), 180));
      document.getElementById('refresh-dashboard')?.addEventListener('click', loadDashboard);
      document.getElementById('refresh-ai-growth-plan')?.addEventListener('click', refreshAiGrowthPlanOnly);
      document.getElementById('copy-ai-growth-plan')?.addEventListener('click', copyAiGrowthPlan);
      document.querySelectorAll('[data-ai-task-route]').forEach(link => link.addEventListener('click', (event) => {
        const route = normalizeAdminRoute(link.dataset.aiTaskRoute || link.getAttribute('href')) || '/admin/dashboard';
        event.preventDefault();
        window.location.assign(route);
      }));
      document.getElementById('agenda-filter')?.addEventListener('input', renderAgendaTable);
      document.querySelectorAll('[data-agenda-dashboard-view]').forEach(btn => btn.addEventListener('click', () => setAgendaDashboardView(btn.dataset.agendaDashboardView)));
      setAgendaDashboardView(agendaDashboardView);
    ...
    async function saveAppointmentFromDashboard() {
      ...
      const previousStatus = itemById(id)?.status || '';
      try {
        await api.patch(`/agenda/${id}/reschedule`, { startsAt });
        await api.patch(`/agenda/${id}/status`, { status });
        ...
        await loadDashboard();
        if (status && status !== previousStatus) await openDashboardStatusMessage(id, status);
        else await showSuccessModal('Agendamento atualizado', 'As alterações foram salvas e o painel já foi atualizado.');
      } catch (error) {
    ...
    async function loadDashboard() {
      showLoading(
        'Carregando Dashboard PetFunny...',
        'Buscando agenda, financeiro, pacotes, engajamento e Gerente IA antes de liberar a tela.',
        { timeoutMs: DASHBOARD_LOADING_TIMEOUT_MS }
      );

      try {
        const [summaryResult, engagementResult, growthResult] = await Promise.allSettled([
          api.get('/dashboard/summary'),
          withClientTimeout(api.get('/dashboard/engagement'), DASHBOARD_OPTIONAL_TIMEOUT_MS, 'Engajamento 360'),
          withClientTimeout(api.get('/dashboard/ai-growth-plan'), DASHBOARD_OPTIONAL_TIMEOUT_MS, 'Gerente IA de Crescimento')
        ]);

        if (summaryResult.status !== 'fulfilled') {
          throw summaryResult.reason || new Error('Não foi possível carregar o resumo do dashboard.');
        }

        const summary = summaryResult.value || {};

        if (engagementResult.status === 'fulfilled') {
          const engagementPayload = engagementResult.value || {};
          summary.engagementDashboard = engagementPayload.engagementDashboard || engagementPayload;
        } else {
          console.warn('[dashboard] Engajamento 360 indisponível:', engagementResult.reason?.message || engagementResult.reason);
          summary.engagementDashboard = fallbackEngagementDashboard('Engajamento 360 demorou ou ficou indisponível. O restante do dashboard foi carregado normalmente.');
        }

        if (growthResult.status === 'fulfilled') {
          summary.aiGrowthPlan = growthResult.value;
        } else {
          console.warn('[dashboard] Gerente IA de crescimento indisponível:', growthResult.reason?.message || growthResult.reason);
          summary.aiGrowthPlan = fallbackAiGrowthPlan('O Gerente IA demorou para responder. O dashboard abriu com um plano seguro local; use Atualizar IA no card para tentar novamente sem travar a tela.');
        }
        ...
        await waitForDashboardVisualReady();
        finishPageLoading();
        hideLoading();
      }
    }
