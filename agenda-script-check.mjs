      ...
      <dialog class="modal-shell agenda-appointment-modal" id="appointment-modal">
        <form method="dialog" class="modal-card agenda-appointment-modal-card" id="appointment-form">
          ...
          <div class="modal-body custom-scroll">
            ...
            <div class="module-card stack-sm soft-card">
            ...
            <label class="form-field"><span>Observações</span><textarea class="textarea" id="notes" rows="4" placeholder="Preferências, restrições, combinados com o tutor..."></textarea></label>
  ...
  function renderServiceChecks() {
    ...
    fields.services.innerHTML = services.length ? Array.from(byCategory.values()).map(group => `
      <div class="service-type-group">
        <div class="service-type-group-head"><strong>${esc(group.name)}</strong><small>${group.items.length} serviço(s) compatíveis com este pet</small></div>
  ...
  async function lookupClientByWhatsapp() {
    ...
    try {
      ...
      if (data.pets.length === 1) {
        fields.petId.value = data.pets[0].id;
        renderServiceChecks();
      }
  ...
  function appointmentCard(a) {
    ...
    return `<article class="appointment-card module-card status-tinted-card" draggable="true" data-id="${esc(a.id)}" data-status="${esc(a.status)}" style="--status-color:${esc(color)}">
      <div class="card-kebab-wrap"><button class="kebab-btn" type="button">⋯</button><div class="card-menu"><button class="edit-appointment" data-id="${esc(a.id)}">Editar</button><button data-whatsapp-action="confirmacao_agendamento" data-appointment-id="${esc(a.id)}">WhatsApp confirmação</button><button data-whatsapp-action="lembrete_agendamento" data-appointment-id="${esc(a.id)}">WhatsApp lembrete</button><button class="status-action" data-id="${esc(a.id)}" data-status="em_atendimento">Check-in</button><button class="status-action" data-id="${esc(a.id)}" data-status="finalizado">Check-out</button><button class="status-action" data-id="${esc(a.id)}" data-status="cancelado">Cancelar</button></div></div>
      <span class="status-pill" style="--status-color:${esc(color)}">${esc(st.name || a.status)}</span>
      <h3>${hm(a.startsAt)} · ${esc(a.petName || 'Pet')}</h3>
      <p><strong>${esc(a.tutorName || 'Tutor')}</strong><br>${esc(a.services || 'Serviço não informado')}</p>
      <div class="page-heading-row between"><small>${esc(a.collaboratorName || 'Equipe PetFunny')}<span class="payment-info-line">${esc(paymentStatusName(a.paymentStatus))}${a.paymentMethodName ? ` · ${esc(a.paymentMethodName)}` : ''}</span></small><b>${money(a.totalCents)}</b></div>
    </article>`;
  }
  ...
  function calendarEvent(a, compact = false) {
    ...
    return `<button draggable="true" class="cal-event edit-appointment" style="--status-color:${esc(statusColor(a.status))}" data-id="${esc(a.id)}" data-start="${esc(a.startsAt)}"><strong>${compact ? `${hm(a.startsAt)} ${esc(a.petName)}` : esc(a.petName)}</strong><small>${esc(a.tutorName)}${compact ? '' : ` · ${money(a.totalCents)}`}<span class="payment-info-line">${esc(paymentLine)}</span></small></button>`;
  }
  ...
  function renderDay() {
    ...
    nodes.day.innerHTML = times.length ? times.map(t => {
      ...
      return `<div class="time-cell">${esc(t)}</div><div class="slot-cell calendar-drop-target" data-date="${esc(state.date)}" data-slot="${esc(t)}">${hourItems.map(a => calendarEvent(a)).join('') || '<small>Horário livre</small>'}</div>`;
  ...
  function renderWeek() {
    ...
    nodes.week.innerHTML = '<div class="time-cell">Hora</div>' + days.map(d => `<div class="time-cell"><strong>${new Date(`${d}T12:00:00`).toLocaleDateString('pt-BR',{weekday:'short',day:'2-digit'})}</strong></div>`).join('') + times.map(t => `<div class="time-cell">${esc(t)}</div>${days.map(d => {
      const items = filteredItems().filter(a => String(a.startsAt).slice(0,10) === d && hm(a.startsAt) === t);
      return `<div class="slot-cell calendar-drop-target" data-date="${esc(d)}" data-slot="${esc(t)}">${items.map(a => calendarEvent(a)).join('')}</div>`;
    }).join('')}`).join('');
  }
  ...
  function renderMonth() {
    ...
    nodes.month.innerHTML = days.map(d => {
      ...
      return `<div class="calendar-day calendar-drop-target ${d.getMonth() !== month ? 'muted' : ''} ${iso === new Date().toISOString().slice(0,10) ? 'today' : ''}" data-date="${esc(iso)}"><div class="calendar-date"><span>${d.getDate()}</span><span class="calendar-count">${items.length}</span></div>${items.slice(0,3).map(a => calendarEvent(a, true)).join('')}</div>`;
    }).join('');
  }
  ...
  async function openForm(item = null) {
    ...
    fields.date.value = item?.startsAt ? String(item.startsAt).slice(0,10) : state.date;
    renderTimes();
    fields.time.value = item?.startsAt ? hm(item.startsAt) : (fields.time.options[0]?.value || '');
  ...
  async function saveAppointment(event) {
    ...
    try {
      showLoading('Salvando agendamento...', 'Validando slots e atualizando a agenda.');
      if (fields.id.value) await api.put(`/agenda/${fields.id.value}`, payload); else await api.post('/agenda', payload);
      nodes.modal.close();
  ...
  fields.tutorId?.addEventListener('change', () => { const tutor = state.tutors.find(t => t.id === fields.tutorId.value); if (fields.whatsapp) fields.whatsapp.value = tutor?.whatsapp || fields.whatsapp.value; renderPetsForTutor(); });
  fields.petId?.addEventListener('change', renderServiceChecks);
